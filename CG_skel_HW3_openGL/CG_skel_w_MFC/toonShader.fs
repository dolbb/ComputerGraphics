...
vec3 calculateDirectionalLight(DirectionalLight directionalLight, vec3 normal, vec3 viewDirection, Material material)
{
	...
	float diffLevel = floor(diffuseFactor * toonLevels);
	diffuseFactor = diffLevel / toonLevels;
	...
	float specLevel = floor(specFactor * toonLevels);
	specFactor = specLevel / toonLevels;
